...
from widgets.str_entry import StrEntry
...
class SetupFrame(ttk.Frame):
    ...
    def __init__(self, master: tk.Tk | ttk.Frame) -> None:
        ...
        # Disabled until extra time option is selected
        self.inputs["ot_time"] = IntEntry(self, "Extra time per half", False,
                                          1)
        self.inputs["ot_time"].grid(row=2, column=1, sticky="NSWE",
                                    padx=self.pad, pady=self.pad)
        self.inputs["ot_time"].disable()

        # Length of the break between overtime halves
        # Disabled until extra time option is selected
        ...
        # Starts the game
        self.start_btn = ttk.Button(self, text="Start Game",
                                    command=self.start)
    ...
    def update(self) -> None:
        ...
        if self.inputs["overtime"].get() == "Extra Time":
            ...
            if ot_time != "" and ot_break != "" and "total" in locals():
                ...
                text = f"Total Game Length: {total}-{ot_total} min"
